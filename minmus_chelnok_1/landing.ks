...
function RotateOrbit {
  ...
  lock throttle to GeoDist(corrtgt, vcrs(body:position, velocity:orbit))/500.
...
// acceleration changes from a0 to a1 at tau, then to a2 at ts
  ...
  local a2 to availablethrust/endmass - body:mu/(body:radius + hland)^2.
...
function VertDescent {
  ...
  until status = "Landed" {
    ...
    lock steering to lookdirup(up:vector - 0.2*min(1, groundspeed)*vh, upfacing).
...
function landing {
  ...
  if lmode = 1 {
    print "landing stage 1 - wait orient".
    local dv to 320*9.80665*ln(mass/(mass-stage:liquidfuel/90))-5.
    waitorient(landsite, arcsin(min(1,dv/velocity:orbit:mag))*0.8).
    nextlmode().
  }

  if lmode = 2 {
    print "landing stage 2 - rotate orbit".
  ...
  if lmode = 3 {
    print "landing stage 3 calculate stop time".
    wait 1.
    stage.
  ...
  if lmode = 4 {
    //clearscreen.
    print "landing stage 4 - gorizontal descent".
    if not gear { gear on. }
    set uf to Descent(landsite, stoptime, shipheight, Isp).
    nextlmode().
  }

  if lmode = 5 {
    print "landing stage 5 vertical descent".
    VertDescent(shipheight, uf).
    lock steering to lookdirup(up:vector, uf).
    wait 5.
    nextlmode().
  }
